...
namespace UnityEngine.UI
{
    ...
    public class Image : MaskableGraphic, ISerializationCallbackReceiver, ILayoutElement, ICanvasRaycastFilter
    {
        ...
        /// </summary>
        [SerializeField]
        private Color m_OverlayColor = DEFAULT_OVERLAY_COLOR;
        public Color overlayColor { get { return m_OverlayColor; } set { if(SetPropertyUtility.SetColor(ref m_OverlayColor, value)) { SetVerticesDirty(); } } }

        /// add slice img fill enabled
        [SerializeField]
        private bool m_SlicedFilled = false;
        public bool slicedFilled { get { return m_SlicedFilled; } set { if (SetPropertyUtility.SetStruct(ref m_SlicedFilled, value)) SetVerticesDirty(); } }

        private static bool m_EnablePolygon = true;
        public bool enablePolygon { get { return m_EnablePolygon; } set { if (SetPropertyUtility.SetStruct(ref m_EnablePolygon, value)) SetVerticesDirty(); } }
        ...
        protected override void OnPopulateMesh(VertexHelper toFill)
        {
                ...
                case Type.Simple:
                    {
                        ...
                        {
                            GeneratePolygonSprite(toFill, m_PreserveAspect);
                        }
                        break;
                    }
                case Type.Sliced:
            ...
            ApplyOverlayColorAndGrey(toFill);
        }
        ...
        protected void ApplyOverlayColorAndGrey(VertexHelper toFill)
        {
            if(m_Grey == false && m_OverlayColor == DEFAULT_OVERLAY_COLOR)
            {
                return;
            }
            toFill.SetUINormal(new Vector3(m_OverlayColor.r, m_OverlayColor.g, m_OverlayColor.b));
            Vector2 uv1 = new Vector2(m_OverlayColor.a, (m_Grey == true) ? 1 : 0);
            toFill.SetUIVertexUv1(uv1);
        }

        /// <summary>
        /// Generate polygon vertices for a simple Image.
        /// </summary>
        void GeneratePolygonSprite(VertexHelper vh, bool lPreserveAspect)
        {
        ...
        private Vector3 CalPosition(Vector4 bound, float x, float y)
        {
            ...
            switch (m_Rotation)
            {
                case Image.Rotation.FlipHorizontal:
                    {
                        pos.x = Mathf.Lerp(bound.z, bound.x, x);
                        pos.y = Mathf.Lerp(bound.y, bound.w, y);
                        break;
                    }
                case Image.Rotation.FlipVertical:
                    {
                        pos.x = Mathf.Lerp(bound.x, bound.z, x);
                        pos.y = Mathf.Lerp(bound.w, bound.y, y);
                        break;
                    }
                case Image.Rotation.Rotation90:
                    {
                        pos.x = Mathf.Lerp(bound.x, bound.z, y);
                        pos.y = Mathf.Lerp(bound.w, bound.y, x);
                        break;
                    }
                case Image.Rotation.Rotation180:
                    {
                        pos.x = Mathf.Lerp(bound.z, bound.x, x);
                        pos.y = Mathf.Lerp(bound.w, bound.y, y);
                        break;
                    }
                case Image.Rotation.Rotation270:
                    {
                        pos.x = Mathf.Lerp(bound.z, bound.x, y);
        ...
        private void GenerateSlicedSprite(VertexHelper toFill)
        {
            ...
            // 增加范围判断 
            Vector4 clippedVert = new Vector4(rect.x, rect.y, rect.x + rect.width, rect.y + rect.height);
            Vector4 clippedUV = new Vector4(outer.x, outer.y, outer.z, outer.w);
            int startGridX = 0, startGridY = 0, endGridX = 2, endGridY = 2;
            float broadVertValue = 0.0f, broadUVValue = 0.0f;
            bool canFill = false;
            if (m_SlicedFilled)
            {
                if (m_FillAmount == 0)
                    ...
                    if (m_FillMethod == FillMethod.Horizontal || m_FillMethod == FillMethod.Vertical)
                    {
                        ...
                        switch (fillMethod)
                        {
                            case FillMethod.Horizontal:
                                ...
                                {
                                    case 0:
                                        broadVertValue = rect.x + rect.width * m_FillAmount;
                                        while (broadVertValue <= s_VertScratch[endGridX].x)
                                        {
                                            --endGridX;
                                        }
                                        broadUVValue = s_UVScratch[endGridX].x + (s_UVScratch[endGridX + 1].x - s_UVScratch[endGridX].x) * (broadVertValue - s_VertScratch[endGridX].x) / (s_VertScratch[endGridX + 1].x - s_VertScratch[endGridX].x);
                                        break;
                                    case 1:
                                        broadVertValue = (rect.x + rect.width) - rect.width * m_FillAmount;
                                        while (broadVertValue >= s_VertScratch[startGridX + 1].x)
                                        {
                                            ++startGridX;
                                        }
                                        broadUVValue = s_UVScratch[startGridX].x + (s_UVScratch[startGridX + 1].x - s_UVScratch[startGridX].x) * (broadVertValue - s_VertScratch[startGridX].x) / (s_VertScratch[startGridX + 1].x - s_VertScratch[startGridX].x);
                                        break;
                                }
                                break;
                            case FillMethod.Vertical:
                                ...
                                {
                                    case 0:
                                        broadVertValue = rect.y + rect.height * m_FillAmount;
                                        while (broadVertValue <= s_VertScratch[endGridY].y)
                                        {
                                            --endGridY;
                                        }
                                        broadUVValue = s_UVScratch[endGridY].y + (s_UVScratch[endGridY + 1].y - s_UVScratch[endGridY].y) * (broadVertValue - s_VertScratch[endGridY].y) / (s_VertScratch[endGridY + 1].y - s_VertScratch[endGridY].y);
                                        break;
                                    case 1:
                                        broadVertValue = (rect.y + rect.height) - rect.height * m_FillAmount;
                                        while (broadVertValue >= s_VertScratch[startGridY + 1].y)
                                        {
                                            ++startGridY;
                                        }
                                        broadUVValue = s_UVScratch[startGridY].y + (s_UVScratch[startGridY + 1].y - s_UVScratch[startGridY].y) * (broadVertValue - s_VertScratch[startGridY].y) / (s_VertScratch[startGridY + 1].y - s_VertScratch[startGridY].y);
                                        break;
                                }
                                break;
                        }
            ...
            for (int x = startGridX; x <= endGridX; ++x)
            {
                ...
                for (int y = startGridY; y <= endGridY; ++y)
                {
                    ...
                    float vertX1 = s_VertScratch[x].x;
                    float vertY1 = s_VertScratch[y].y;
                    float vertX2 = s_VertScratch[x2].x;
                    float vertY2 = s_VertScratch[y2].y;
                    float uvX1 = s_UVScratch[x].x;
                    float uvY1 = s_UVScratch[y].y;
                    float uvX2 = s_UVScratch[x2].x;
                    float uvY2 = s_UVScratch[y2].y;
                    ...
                    AddQuad(toFill,
                        new Vector2(vertX1, vertY1),
                        new Vector2(vertX2, vertY2),
                        color,
