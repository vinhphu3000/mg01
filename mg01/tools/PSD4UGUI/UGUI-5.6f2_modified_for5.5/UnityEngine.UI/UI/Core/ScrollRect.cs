    ...
    public class ScrollRect : UIBehaviour, IInitializePotentialDragHandler, IBeginDragHandler, IEndDragHandler, IDragHandler, IScrollHandler, ICanvasElement, ILayoutElement, ILayoutGroup
    {
        ...
        public float horizontalNormalizedPosition
        {
            get
            {
                ...
                //源码修改jr.zeng_20170831
                if (m_ContentBounds.size.x <= m_ViewBounds.size.x + 0.01f)  //处理误差
                {
                    //内容比遮罩小
                    if (Mathf.Abs(m_ViewBounds.min.x - m_ContentBounds.min.x) < 0.1)
                    {
                    ...
                    {
                        //拖出范围时
                        return (m_ViewBounds.min.x > m_ContentBounds.min.x) ? 1.1f : -0.9f; //
                    }
                }
        ...
        public float verticalNormalizedPosition
        {
            get
            {
                ...
                //源码修改jr.zeng_20170831
                if (m_ContentBounds.size.y <= m_ViewBounds.size.y + 0.01f)  //处理误差
                {
                    //内容比遮罩小
                    if (Mathf.Abs(m_ViewBounds.min.y - m_ContentBounds.min.y) < 0.1)
                    {
                    ...
                    {
                        //拖出范围时
                        return (m_ViewBounds.min.y > m_ContentBounds.min.y) ? 1.1f : -0.9f; //
                    }
                }
