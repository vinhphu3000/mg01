local clonefunc = require "clonefunc.c"
local next = next
local hardreload = {}
local upvalue_map = {}
...
local function same_proto(f1,f2)
	...
	while true do
		...
		if name ~= "_ENV"  and uv[name] == nil and upvalue_map[name] == nil then
			...
			while true do
				...
				if name2 == nil then
					...
					if not upvalue_map[name] then
						if error_log then error_log("ERROR upvalue", name, nf) end
						-- oldf = f
						break
					end
				...
				if name == name2 then
					if print then print("HEHE upvalue", name, nf) end
