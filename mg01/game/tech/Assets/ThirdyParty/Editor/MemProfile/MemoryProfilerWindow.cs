...
namespace MemoryProfilerWindow
{
    public class MemoryProfilerWindow : EditorWindow
    {
        ...
        void OnGUI()
        {
            ...
            if (GUILayout.Button("Diff Snapshot..."))
            {
                string fileName = EditorUtility.OpenFilePanel("Load Snapshot", null, "memsnap");
                if (!string.IsNullOrEmpty(fileName))
                {
                    ...
                    using (Stream stream = File.Open(fileName, FileMode.Open))
                    {
                        DiffSnapshot(bf.Deserialize(stream) as PackedMemorySnapshot);
                    }
                }
            }
            if (GUILayout.Button("dump to txt"))
            {
                _treeMapView.InitDump();
                System.IO.File.WriteAllBytes("mem.txt", System.Text.Encoding.UTF8.GetBytes(_treeMapView.DumpLable()));
                //System.IO.File.WriteAllBytes("mesh.txt", System.Text.Encoding.UTF8.GetBytes(_treeMapView.DumpGroup("Mesh")));
                //System.IO.File.WriteAllBytes("string.txt", System.Text.Encoding.UTF8.GetBytes(_treeMapView.DumpGroup("System.String")));
            }
        ...
        void DiffSnapshot(PackedMemorySnapshot snapshot)
        {
            ...
            var dirtyObjectsy = snapshot.nativeObjects.ToDictionary(td=>td.instanceId, td => 1);
            _packedCrawled = new Crawler().Crawl(_snapshot);
            Unpack(dirtyObjectsy);
        }
    }
}
