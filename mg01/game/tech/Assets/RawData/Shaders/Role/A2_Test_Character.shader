Shader "A2/Test-Character-lobby" {
    Properties {
    ...
    SubShader {
		...
		Pass {
			...
			float4 frag(VertexOutput i) :COLOR {
				...
				float node_4011 = 0.5*dot(viewDirection,normalDirection)+0.5;
                float3 finalColor = (lerp((_RimColor.rgb+_OutlineColor.rgb),_OutlineColor.rgb,pow(saturate((node_4011*_Edge)),_Soft)));
        ...
        Pass {
            ...
            float4 frag(VertexOutput i) : COLOR {
				...
				float3 reflectDir = (negViewDir - (2.0 * (dot(worldNormal, negViewDir)* worldNormal)));
				...
				finalColor.xyz = (((diffuse_7*mainColor.xyz) + specular_2) + emissive_4);
